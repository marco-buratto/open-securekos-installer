...
class System:
    ...
    def getDiskDevices():
        ...
        try:
            # udev watch & parse (only) for *ata devices.
            context = pyudev.Context()
            for device in context.list_devices(subsystem='block',ID_BUS='ata'):
    ...
    @staticmethod
    def wipeDevices(device, debugMode):
        ...
        if debugMode=="true":
            Util.debugMessage(["wipeDevices",planeCmdln,createBlankGPTCommand],device)
    ...
    def writeUEFIStructures(device,partitionNumber,uefiPartitionMB,isoUuidSystemPartition,isoUuidSecondSystemPartition,encryptionSecret,debugMode):
        ...
        if int(uefiPartitionMB):
            # Create UEFI structure (must be FAT).
